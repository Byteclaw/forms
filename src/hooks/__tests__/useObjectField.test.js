  ...
  return (
    <Fragment>
      ...
      <button
        ...
        onClick={(e: any) => field.setField(e.target.field, e.target.value)}
        type="button"
      >
        Set
      </button>
...
describe('useObjectField hook', () => {
  it('works corretly', async () => {
    ...
    // $FlowFixMe
    expect(getByTestId('input').value).toBe('{"a":1,"b":2}');
    ...
    expect(getByTestId('dirty').innerHTML).toBe('false');
    // $FlowFixMe
    expect(getByTestId('input').value).toBe('{"a":2,"b":1,"c":3}');
    ...
    expect(getByTestId('changing').innerHTML).toBe('true');
    // $FlowFixMe
    expect(getByTestId('input').value).toBe('{"a":2,"b":1,"c":3,"d":"4"}');
    ...
    expect(getByTestId('changing').innerHTML).toBe('true');
    // $FlowFixMe
    expect(getByTestId('input').value).toBe('{"a":"4","b":1,"c":3,"d":"4"}');
